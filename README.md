...
このリポジトリの特徴は、**セットアップ・運用そのものをエージェントに任せられる**ように設計されていることです。実際にこの環境一式は Claude Code のセッションが自律的に構築したもので、そのとき踏んだ罠(エージェントがエージェントを起動するときの同型ネストセッション衝突など)はすべてスクリプト側で対策し、`team doctor` で診断できるようにしてあります。詳細は [docs/agent-driven-setup.md](docs/agent-driven-setup.md) を参照してください。
...
| `CLAUDE.md` | この repo を開いたエージェントへの案内(セットアップを任せるときの入口) |
| `docs/agent-driven-setup.md` | エージェント主導セットアップのランブックと人間/エージェントの分業 |
