...
.agent/bin/team doctor    # 依存・設定・チーム登録・外側エージェントセッションの診断
.agent/bin/team restart   # 再起動
.agent/bin/team stop      # 停止(Codex の app-server / ブリッジも終了)
```

`team start` は両ペインとも同型ネストセッション対策込みで起動します(claude は
`launch-claude-reviewer.sh` 経由、codex は `env -u CODEX_SANDBOX -u CODEX_THREAD_ID`
前置)。エージェント自身にセットアップや起動を任せる場合の全体像は
[docs/agent-driven-setup.md](../../docs/agent-driven-setup.md) を参照してください。
